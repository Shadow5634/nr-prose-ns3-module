...
main(int argc, char* argv[])
{
    ...
    uint8_t bwpIdInNet = 1; // BWP1 will be the BWP for in-network flows and in-network bearers will
                            // be setup with 'GBR_CONV_VOICE' QCI
    ...
    bwp.rbPerRbg = 1;
    bwp.bandwidth =
        bandwidthCc0Bpw0 / 1000 / 100; // SL configuration requires BW in Multiple of 100 KHz
    ...
    for (uint32_t i = 0; i < slUeNodes.GetN() - 1; ++i)
    {
        for (uint32_t j = i + 1; j < slUeNodes.GetN(); ++j)
        {
            ...
            NS_LOG_INFO("OnOff application installed in UE nodeId "
                        << i << " srcIp " << slIpv4AddressVector[i] << " towards UE nodeId " << j
                        << " dstIp " << slIpv4AddressVector[j]);
            ...
            if (bidirectionalSlTraffic)
            {
                ...
                slClientApps.Add(app);
                NS_LOG_INFO("OnOff application installed in UE nodeId "
                            << j << " srcIp " << slIpv4AddressVector[j] << " towards UE nodeId "
                            << i << " dstIp " << slIpv4AddressVector[i]);
