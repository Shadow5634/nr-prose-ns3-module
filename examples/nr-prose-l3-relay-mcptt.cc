...
struct NetSimulyzerMcpttFlow
{
    ...
    std::map<uint32_t, Ptr<netsimulyzer::ThroughputSink>>
        m_rxTputSeriesPerRx; ///< Set of curves for the received throughput from the transmitter,
                             ///< one per receiver
    std::map<uint32_t, Ptr<netsimulyzer::XYSeries>>
        m_rxDelaySeriesPerRx; ///< Set of curves for the media packet delay from the transmitter,
                              ///< one per receiver
    std::map<uint32_t, Ptr<netsimulyzer::EcdfSink>>
        m_rxDelayEcdfPerRx; ///< Set of eCDFs for the media packet delay from the transmitter, one
                            ///< per receiver
};
 ...
 */
void
NetSimulyzerMcpttFlow::TxTrace(Ptr<const Application> app,
                               uint16_t callId,
                               Ptr<const Packet> p,
                               const TypeId& headerType)
{
 ...
 */
void
NetSimulyzerMcpttFlow::RxTrace(Ptr<const Application> app,
                               uint16_t callId,
                               Ptr<const Packet> p,
                               const TypeId& headerType)
{
    if (headerType == McpttMediaMsg::GetTypeId())
    {
        ...
        if (m_txNodeId == txingNodeId)
        {
            ...
            std::map<uint32_t, Ptr<netsimulyzer::ThroughputSink>>::iterator it =
                m_rxTputSeriesPerRx.find(thisNodeId);
            ...
            Time timeStamp = MicroSeconds(rtpHeader.GetTimestamp() *
                                          10); // RTP timestamp impl is in 10ths of microseconds
            double delay =
                Simulator::Now().GetSeconds() * 1000.0 - timeStamp.GetSeconds() * 1000.0; // ms

            // Find delay XYSeries
            std::map<uint32_t, Ptr<netsimulyzer::XYSeries>>::iterator it2 =
                m_rxDelaySeriesPerRx.find(thisNodeId);
            ...
            // Find delay eCDF
            std::map<uint32_t, Ptr<netsimulyzer::EcdfSink>>::iterator it3 =
                m_rxDelayEcdfPerRx.find(thisNodeId);
...
struct NetSimulyzerMcpttMetricsTracer
{
    ...
    std::map<uint32_t, Ptr<netsimulyzer::XYSeries>>
        m_m2eLatencyTimelineSeriesPerCall; ///< Set of M2E latency timeline curves, one per call
    std::map<uint32_t, Ptr<netsimulyzer::EcdfSink>>
        m_m2eLatencyEcdfPerCall; ///< Set of M2E eCDFs, one per call
    std::map<uint32_t, Ptr<netsimulyzer::XYSeries>>
        m_accessTimeTimelineSeriesPerCall; ///< Set of M2E timeline curves, one per call
    std::map<uint32_t, Ptr<netsimulyzer::EcdfSink>>
        m_accessTimeEcdfPerCall; ///< Set of M2E eCDFs, one per call
};
                                                ...
                                                Time latency)
{
    std::map<uint32_t, Ptr<netsimulyzer::XYSeries>>::iterator it =
        m_accessTimeTimelineSeriesPerCall.find(callId);
    ...
    std::map<uint32_t, Ptr<netsimulyzer::EcdfSink>>::iterator it2 =
        m_accessTimeEcdfPerCall.find(callId);
...
{
    std::map<uint32_t, Ptr<netsimulyzer::XYSeries>>::iterator it =
        m_m2eLatencyTimelineSeriesPerCall.find(callId);
    ...
    std::map<uint32_t, Ptr<netsimulyzer::EcdfSink>>::iterator it2 =
        m_m2eLatencyEcdfPerCall.find(callId);
...
main(int argc, char* argv[])
{
    ...
    std::vector<std::map<uint32_t, NetSimulyzerMcpttFlow>> netSimulyzerFlowsPerGroup(
        groupVectorsSize);
    ...
    std::vector<Ptr<netsimulyzer::SeriesCollection>> rxDelayEcdfCollectionsPerGroup(
        groupVectorsSize);
    std::vector<Ptr<netsimulyzer::SeriesCollection>> rxDelayTimelineCollectionsPerGroup(
        groupVectorsSize);
    std::vector<Ptr<netsimulyzer::SeriesCollection>> txTputTimelineCollectionsPerGroup(
        groupVectorsSize);
    std::vector<Ptr<netsimulyzer::SeriesCollection>> rxTputTimelineCollectionsPerGroup(
        groupVectorsSize);

    for (uint32_t groupId = 1; groupId < groupVectorsSize; ++groupId)
    {
        txTputTimelineCollectionsPerGroup[groupId] =
            CreateObject<netsimulyzer::SeriesCollection>(orchestrator);
        txTputTimelineCollectionsPerGroup[groupId]->SetAttribute(
            "Name",
            StringValue("Throughput - Timeline - Tx - CallId " + std::to_string(groupId)));
        ...
        txTputTimelineCollectionsPerGroup[groupId]->SetAttribute("HideAddedSeries",
                                                                 BooleanValue(true));

        rxTputTimelineCollectionsPerGroup[groupId] =
            CreateObject<netsimulyzer::SeriesCollection>(orchestrator);
        rxTputTimelineCollectionsPerGroup[groupId]->SetAttribute(
            "Name",
            StringValue("Throughput - Timeline - Rx - CallId " + std::to_string(groupId)));
        ...
        yAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name",
                                                           StringValue("Throughput (kb/s)"));
        rxTputTimelineCollectionsPerGroup[groupId]->SetAttribute("HideAddedSeries",
                                                                 BooleanValue(true));

        rxDelayTimelineCollectionsPerGroup[groupId] =
            CreateObject<netsimulyzer::SeriesCollection>(orchestrator);
        rxDelayTimelineCollectionsPerGroup[groupId]->SetAttribute(
            "Name",
            StringValue("Media packet delay - Timeline - CallId " + std::to_string(groupId)));
        ...
        yAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name",
                                                           StringValue("Media packet delay (ms)"));
        rxDelayTimelineCollectionsPerGroup[groupId]->SetAttribute("HideAddedSeries",
                                                                  BooleanValue(true));

        rxDelayEcdfCollectionsPerGroup[groupId] =
            CreateObject<netsimulyzer::SeriesCollection>(orchestrator);
        rxDelayEcdfCollectionsPerGroup[groupId]->SetAttribute(
            "Name",
            StringValue("Media packet delay - eCDF - CallId " + std::to_string(groupId)));
        rxDelayEcdfCollectionsPerGroup[groupId]->GetAttribute("XAxis", xAxis);
        xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name",
                                                           StringValue("Media packet delay (ms)"));
        rxDelayEcdfCollectionsPerGroup[groupId]->SetAttribute("HideAddedSeries",
                                                              BooleanValue(true));
    }
    ...
    for (uint32_t groupId = 1; groupId < groupVectorsSize; ++groupId)
    {
        // First add all transmitters -> create flows and setup transmitter trace for all UEs in
        // group
        for (auto& txNodeId : nodeIdsPerGroup[groupId])
        {
            NetSimulyzerMcpttFlow flow;
            flow.m_txNodeId = txNodeId;
            flow.m_txTputSeries = CreateObject<netsimulyzer::ThroughputSink>(
                orchestrator,
                "Call ID " + std::to_string(groupId) + " - Flow Tx - Tx Id " +
                    std::to_string(txNodeId));
            flow.m_txTputSeries->GetSeries()->SetAttribute("Color", GetNextColor());
            netSimulyzerFlowsPerGroup[groupId].insert(
                std::pair<uint32_t, NetSimulyzerMcpttFlow>(txNodeId, flow));
        }
        // Then each UE adds itself as a receiver to all transmitter in the group  -> setup receiver
        // traces
        for (auto& rxNodeId : nodeIdsPerGroup[groupId])
        {
            for (std::map<uint32_t, NetSimulyzerMcpttFlow>::iterator it =
                     netSimulyzerFlowsPerGroup[groupId].begin();
                 it != netSimulyzerFlowsPerGroup[groupId].end();
                 ++it)
            {
                ...
                // Rx throughput
                auto rxTputSeries = CreateObject<netsimulyzer::ThroughputSink>(
                    orchestrator,
                    "Call ID " + std::to_string(groupId) + " - Flow Rx - Tx Id " +
                        std::to_string(it->first) + " - Rx Id " + std::to_string(rxNodeId));
                rxTputSeries->GetSeries()->SetAttribute("Color", GetNextColor());
                it->second.m_rxTputSeriesPerRx.insert(
                    std::pair<uint32_t, Ptr<netsimulyzer::ThroughputSink>>(rxNodeId, rxTputSeries));

                // Media packet delay Timeline
                auto rxDelaySeries = CreateObject<netsimulyzer::XYSeries>(orchestrator);
                rxDelaySeries->SetAttribute("Name",
                                            StringValue("Call ID " + std::to_string(groupId) +
                                                        " - Flow Rx - Tx Id " +
                                                        std::to_string(it->first) + " - Rx Id " +
                                                        std::to_string(rxNodeId)));
                ...
                rxDelaySeries->GetYAxis()->SetAttribute("Name",
                                                        StringValue("Media packet delay (ms)"));
                it->second.m_rxDelaySeriesPerRx.insert(
                    std::pair<uint32_t, Ptr<netsimulyzer::XYSeries>>(rxNodeId, rxDelaySeries));

                // Media packet delay eCDF
                auto rxDelayEcdf = CreateObject<netsimulyzer::EcdfSink>(
                    orchestrator,
                    "Call ID " + std::to_string(groupId) + " - Flow Rx - Tx Id " +
                        std::to_string(it->first) + " - Rx Id " + std::to_string(rxNodeId));
                ...
                it->second.m_rxDelayEcdfPerRx.insert(
                    std::pair<uint32_t, Ptr<netsimulyzer::EcdfSink>>(rxNodeId, rxDelayEcdf));
            }
        }
             ...
             ++it)
        {
            ...
            tputCollection->SetAttribute("Name",
                                         StringValue("Throughput - Timeline - CallId " +
                                                     std::to_string(groupId) + " - Tx Id " +
            ...
            for (std::map<uint32_t, Ptr<netsimulyzer::ThroughputSink>>::iterator it2 =
                     it->second.m_rxTputSeriesPerRx.begin();
                 it2 != it->second.m_rxTputSeriesPerRx.end();
                 ++it2)
            {
            ...
            delaySeriesCollection->SetAttribute(
                "Name",
                StringValue("Media packet delay - Timeline - CallId " + std::to_string(groupId) +
            ...
            for (std::map<uint32_t, Ptr<netsimulyzer::XYSeries>>::iterator it3 =
                     it->second.m_rxDelaySeriesPerRx.begin();
                 it3 != it->second.m_rxDelaySeriesPerRx.end();
                 ++it3)
            {
            ...
            delayEcdfCollection->SetAttribute("Name",
                                              StringValue("Media packet delay - eCDF - CallId " +
                                                          std::to_string(groupId) + " - Tx Id " +
                                                          std::to_string(it->first)));
            delayEcdfCollection->GetAttribute("XAxis", xAxis);
            xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute(
                "Name",
                StringValue("Media packet delay (ms)"));

            for (std::map<uint32_t, Ptr<netsimulyzer::EcdfSink>>::iterator it4 =
                     it->second.m_rxDelayEcdfPerRx.begin();
                 it4 != it->second.m_rxDelayEcdfPerRx.end();
                 ++it4)
            {
    ...
    for (uint32_t inIdx = 0; inIdx < inNetUeNodes.GetN(); ++inIdx)
    {
        ...
        std::map<uint32_t, NetSimulyzerMcpttFlow>::iterator it =
            netSimulyzerFlowsPerGroup[groupId].find(nodeId);
        pttApp->TraceConnectWithoutContext(
            "TxTrace",
            MakeCallback(&NetSimulyzerMcpttFlow::TxTrace, &it->second));

        // Connect reception trace source to the other transmitters in the group
        for (std::map<uint32_t, NetSimulyzerMcpttFlow>::iterator it2 =
                 netSimulyzerFlowsPerGroup[groupId].begin();
             it2 != netSimulyzerFlowsPerGroup[groupId].end();
             ++it2)
        {
            if (it2->first == nodeId)
            {
            ...
            else
            {
                pttApp->TraceConnectWithoutContext(
                    "RxTrace",
                    MakeCallback(&NetSimulyzerMcpttFlow::RxTrace, &it2->second));
            }
        }
#endif
        ...
        std::cout << " UL: " << inNetIpv4AddressVector[inIdx] << " -> " << serverAddress
                  << std::endl;
        std::cout << " DL: " << serverAddress << " -> " << inNetIpv4AddressVector[inIdx]
                  << std::endl;
    ...
    if (relayUesTraffic)
    {
        ...
        for (uint32_t ryIdx = 0; ryIdx < relayUeNodes.GetN(); ++ryIdx)
        {
            ApplicationContainer pttAppContainer =
                mcpttClientHelper.Install(relayUeNodes.Get(ryIdx));
            Ptr<psc::McpttPttApp> pttApp = pttAppContainer.Get(0)->GetObject<psc::McpttPttApp>();
            ...
            std::cout << " UL: " << relaysIpv4AddressVector[ryIdx] << " -> " << serverAddress
                      << std::endl;
            std::cout << " DL: " << serverAddress << " -> " << relaysIpv4AddressVector[ryIdx]
                      << std::endl;
    ...
    for (uint32_t rmIdx = 0; rmIdx < remoteUeNodes.GetN(); ++rmIdx)
    {
        ...
        std::cout << " UL: " << remotesIpv4AddressVector[rmIdx] << " -> " << serverAddress
                  << std::endl;
        std::cout << " DL: " << serverAddress << " -> " << remotesIpv4AddressVector[rmIdx]
                  << std::endl;
    ...
    Ptr<psc::McpttPusherOrchestratorSpurtCdf> groupOneSpurtOrchestrator =
        CreateObject<psc::McpttPusherOrchestratorSpurtCdf>();
    groupOneSpurtOrchestrator->SetAttribute("ActivityFactor", DoubleValue(vaf));
    groupOneMcpttOrchestrator = groupOneSpurtOrchestrator;

    Ptr<psc::McpttPusherOrchestratorContention> groupOneContentionOrchestrator =
        CreateObject<psc::McpttPusherOrchestratorContention>();
    groupOneContentionOrchestrator->SetAttribute("ContentionProbability", DoubleValue(cp));
    groupOneContentionOrchestrator->SetAttribute("Orchestrator",
                                                 PointerValue(groupOneMcpttOrchestrator));
    groupOneMcpttOrchestrator = groupOneContentionOrchestrator;

    Ptr<psc::McpttPusherOrchestratorSessionCdf> groupOneSessionOrchestrator =
        CreateObject<psc::McpttPusherOrchestratorSessionCdf>();
    groupOneSessionOrchestrator->SetAttribute("ActivityFactor", DoubleValue(saf));
    groupOneSessionOrchestrator->SetAttribute("Orchestrator",
                                              PointerValue(groupOneMcpttOrchestrator));
    ...
    Ptr<psc::McpttPusherOrchestratorSpurtCdf> groupTwoSpurtOrchestrator =
        CreateObject<psc::McpttPusherOrchestratorSpurtCdf>();
    groupTwoSpurtOrchestrator->SetAttribute("ActivityFactor", DoubleValue(vaf));
    groupTwoMcpttOrchestrator = groupTwoSpurtOrchestrator;

    Ptr<psc::McpttPusherOrchestratorContention> groupTwoContentionOrchestrator =
        CreateObject<psc::McpttPusherOrchestratorContention>();
    groupTwoContentionOrchestrator->SetAttribute("ContentionProbability", DoubleValue(cp));
    groupTwoContentionOrchestrator->SetAttribute("Orchestrator",
                                                 PointerValue(groupTwoMcpttOrchestrator));
    groupTwoMcpttOrchestrator = groupTwoContentionOrchestrator;

    Ptr<psc::McpttPusherOrchestratorSessionCdf> groupTwoSessionOrchestrator =
        CreateObject<psc::McpttPusherOrchestratorSessionCdf>();
    groupTwoSessionOrchestrator->SetAttribute("ActivityFactor", DoubleValue(saf));
    groupTwoSessionOrchestrator->SetAttribute("Orchestrator",
                                              PointerValue(groupTwoMcpttOrchestrator));
    ...
    groupOneIpv4Addresses.insert(groupOneIpv4Addresses.begin(),
                                 inNetIpv4AddressVector.begin(),
                                 inNetIpv4AddressVector.end());
    groupOneIpv4Addresses.insert(groupOneIpv4Addresses.begin(),
                                 relaysIpv4AddressVector.begin(),
                                 relaysIpv4AddressVector.end());

    ApplicationContainer groupTwoClientApps;
    groupTwoClientApps.Add(remoteClientApps);
    groupTwoIpv4Addresses.insert(groupTwoIpv4Addresses.begin(),
                                 remotesIpv4AddressVector.begin(),
                                 remotesIpv4AddressVector.end());
    ...
    groupId = 1;
    callHelper.AddCall(groupOneClientApps,
                       serverApp,
                       groupId,
                       psc::McpttCallMsgFieldCallType::BASIC_GROUP,
                       timeStartTraffic,
                       simTime - Seconds(1.0));
    ipv4addressPerGroup.insert(
        std::pair<uint32_t, std::vector<Ipv4Address>>(groupId, groupOneIpv4Addresses));

    groupId = 2;
    callHelper.AddCall(groupTwoClientApps,
                       serverApp,
                       groupId,
                       psc::McpttCallMsgFieldCallType::BASIC_GROUP,
                       timeStartTraffic,
    ...
    ipv4addressPerGroup.insert(
        std::pair<uint32_t, std::vector<Ipv4Address>>(groupId, groupTwoIpv4Addresses));
    ...
    for (uint16_t callId = 1; callId < groupVectorsSize; ++callId) // callId == groupId
    {
        ...
        auto groupAccessTimeEcdf = CreateObject<netsimulyzer::EcdfSink>(
            orchestrator,
            "Access time - eCDF - CallId " + std::to_string(callId));
        ...
        mcpttMetricsTracer.m_accessTimeEcdfPerCall.insert(
            std::pair<uint32_t, Ptr<netsimulyzer::EcdfSink>>(callId, groupAccessTimeEcdf));
        ...
        auto groupM2eLatencyEcdfs = CreateObject<netsimulyzer::EcdfSink>(
            orchestrator,
            "M2E latency - eCDF - CallId " + std::to_string(callId));
        ...
        mcpttMetricsTracer.m_m2eLatencyEcdfPerCall.insert(
            std::pair<uint32_t, Ptr<netsimulyzer::EcdfSink>>(callId, groupM2eLatencyEcdfs));
        ...
        groupAccessTimeTimeline->SetAttribute(
            "Name",
            StringValue("Access time - Timeline - CallId " + std::to_string(callId)));
        ...
        mcpttMetricsTracer.m_accessTimeTimelineSeriesPerCall.insert(
            std::pair<uint32_t, Ptr<netsimulyzer::XYSeries>>(callId, groupAccessTimeTimeline));
        ...
        groupM2eLatencyTimeline->SetAttribute(
            "Name",
            StringValue("M2E latency - Timeline - CallId " + std::to_string(callId)));
        ...
        mcpttMetricsTracer.m_m2eLatencyTimelineSeriesPerCall.insert(
            std::pair<uint32_t, Ptr<netsimulyzer::XYSeries>>(callId, groupM2eLatencyTimeline));
    ...
    *RelayNasRxPacketTraceStream->GetStream()
    ...
    for (std::map<uint32_t, std::vector<Ipv4Address>>::const_iterator itGroupAddr =
             ipv4addressPerGroup.begin();
         itGroupAddr != ipv4addressPerGroup.end();
         ++itGroupAddr)
    {
        outFile << "#Type\tcallId\tsrcIpd\tdstIp\tnTxPackets\tnRxPackets\tLossRatio\tMeanDelay(ms)"
                   "\tMeanJitter(ms)\n";

        for (std::vector<Ipv4Address>::const_iterator itAddr = itGroupAddr->second.begin();
             itAddr != itGroupAddr->second.end();
             itAddr++)
        {
            // Media packets first
                 ...
                 ++i)
            {
                ...
                if (t.sourceAddress == *itAddr || t.destinationAddress == *itAddr)
                {
                    ...
                    outFile << (double)(i->second.txPackets - i->second.rxPackets) /
                                   i->second.txPackets
                            << "\t";
