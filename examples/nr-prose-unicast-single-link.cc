...
main(int argc, char* argv[])
{
    ...
    for (uint16_t ac = 0; ac < clientApps.GetN(); ac++)
    {
        ...
        clientApps.Get(ac)->TraceConnect(
            "TxWithSeqTsSize",
            "tx",
            MakeBoundCallback(&UePacketTraceDb, &pktStats, ueVoiceContainer.Get(0), localAddrs));
    }
    ...
    for (uint16_t ac = 0; ac < serverApps.GetN(); ac++)
    {
        ...
        serverApps.Get(ac)->TraceConnect(
            "RxWithSeqTsSize",
            "rx",
            MakeBoundCallback(&UePacketTraceDb, &pktStats, ueVoiceContainer.Get(1), localAddrs));
    }
    ...
    *Pc5SignallingPacketTraceStream->GetStream()
    ...
    std::cout << "Avrg thput = "
              << (rxByteCounter * 8) / (simTime - Seconds(realAppStart)).GetSeconds() / 1000.0
              << " kbps" << std::endl;
