...
//NOTE: The issue here was that the NrSlProseHelper class was not being found
//NOTE: This was fixed by looking at the include files of the other new examples that also used this class
//NEW CODE START
...
//Ptr<McpttTraceHelper> mcpttTraceHelper,
//Ptr<netsimulyzer::SeriesCollection> accessTimeEcdfCollection,
/*
 ...
 */
void
NetSimulyzerMcpttMetricsTracer::AccessTimeTrace(Time t,
                                                uint32_t userId,
                                                uint16_t callId,
                                                std::string result,
                                                Time latency)
{
  ...
  // Only concerned about I or Q for our access time graph
  if(result != "I" && result != "Q")
  {
    return;
  }
  
  //After printing userId and callId the node Ids and userIds seem to match acc:
  //1-4 for on network team 1
  //5-8 for relay team 2
  //9-12 for off network team
  
  uint16_t teamId = 0;
  
  if(userId >= 1 && userId <= 4)
  {
    teamId = 1; // on network call
  }
  else if (userId >= 5 && userId <= 8)
  {
    teamId = 2; // relay+remote call
  }
  else if (userId >= 9 && userId <= 12)
  {
    teamId = 3; // off network call
  }
  else
  {
    NS_FATAL_ERROR("UsrId not recognized");
    return;
  }

  std::map<uint32_t, Ptr<netsimulyzer::XYSeries>>::iterator it =
      m_accessTimeTimelineSeriesPerCall.find(teamId);
  ...
  std::map<uint32_t, Ptr<netsimulyzer::EcdfSink>>::iterator it2 =
      m_accessTimeEcdfPerCall.find(teamId);
 ...
 */
void
NetSimulyzerMcpttMetricsTracer::M2eLatencyTrace(Time t,
                                                uint32_t ssrc,
                                                uint64_t nodeId,
                                                uint16_t callId,
                                                Time latency)
{
  uint16_t teamId = 0;
  uint64_t userId = nodeId; // can use either ssrc/nodeId since - both part of same team
  ...
  std::map<uint32_t, Ptr<netsimulyzer::XYSeries>>::iterator it =
      m_m2eLatencyTimelineSeriesPerCall.find(teamId);
  ...
  std::map<uint32_t, Ptr<netsimulyzer::EcdfSink>>::iterator it2 =
      m_m2eLatencyEcdfPerCall.find(teamId);
...
void trial(  Ptr<netsimulyzer::Orchestrator> orchestrator,
             Ptr<Node> node,
             Ptr<netsimulyzer::NodeConfiguration> nodeConfig,
             Ptr<const Application> app,
             uint16_t callId,
             Ptr<const Packet> packet,
             const TypeId& headerType)
{
  nodeConfig->SetHighlightColor(netsimulyzer::RED);
  //std::cout << nodeConfig->GetHighlightColor() << std::endl;
...
main (int argc, char *argv[])
{
  ...
  //std::cout << std::endl;
  //
  //std::cout << "In network UE nodes; ID range: " << inNetUeNodes.Get (0)->GetId () << " to " << inNetUeNodes.Get (nInNetUes - 1)->GetId () << std::endl;
  //std::cout << "Relay UEs; ID range: " << relayUeNodes.Get (0)->GetId () << " to " << relayUeNodes.Get (nRelayUes - 1)->GetId () << std::endl;
  //std::cout << "Remote UEs; ID range: " << remoteUeNodes.Get (0)->GetId () << " to " << remoteUeNodes.Get (nRemoteUes - 1)->GetId () << std::endl;
  //std::cout << "Off net UEs; ID range: " << offNetUeNodes.Get (0)->GetId () << " to " << offNetUeNodes.Get (nOffNetUes - 1)->GetId () << std::endl;
  ...
  for (uint32_t inNetIdx = 0; inNetIdx < inNetUeNodes.GetN (); ++inNetIdx)
    {
      ...
      //cout << "InNet UE " << inNetIdx + 1 << " node id = [" << inNetUeNodes.Get (inNetIdx)->GetId () << "]"
      //             " x " << xInit + inNetIdx * xInc << " y " << yInit - inNetIdx * yInc << endl;
    }
  ...
  for (uint32_t ryIdx = 0; ryIdx < relayUeNodes.GetN (); ++ryIdx)
    {
      ...
      //cout << "Relay UE " << ryIdx + 1 << " node id = [" << relayUeNodes.Get (ryIdx)->GetId () << "]"
      //             " x " << relayXInit + ryIdx * xInc << " y " << yInit - ryIdx * yInc << endl;
    }
  ...
  for (uint32_t rmIdx = 0; rmIdx < remoteUeNodes.GetN (); ++rmIdx)
    {
      ...
      //cout << "Remote UE " << rmIdx << " node id = [" << remoteUeNodes.Get (rmIdx)->GetId () << "]"
      //              " x " << xInit + rmIdx * xInc << " y " << yInit - rmIdx * yInc << endl;
    }
  ...
  for (uint32_t offNetIdx = 0; offNetIdx < offNetUeNodes.GetN (); ++offNetIdx)
    {
      ...
      //cout << "OffNet UE " << offNetIdx + 1 << " node id = [" << offNetUeNodes.Get (offNetIdx)->GetId () << "]"
      //             " x " << xInit + offNetIdx * xInc << " y " << yInit - offNetIdx * yInc << endl;
    }
  ...
  //START OF REORDERED CODE
  //I shall be reordering some of this acc to the changes in the nr-prose-l3-realay-mcptt.cc file
  ...
  //NEW CODE START
    ...
    //I am setting the NumHarqProcess param here as well since this default is 20 in the new code
    nrUeMacFactory.Set("NumHarqProcess", UintegerValue(20));
    ...
    nrSlUeMacFactory.Set("T2", UintegerValue(t2));
    nrSlUeMacFactory.Set("ActivePoolId", UintegerValue(0));
    
    //NOTE: the file does not set this anywhere
    //NOTE: in the new nr model the default is 16 while in the old model it seems to be 20 and thus I have set it to 20
    nrSlUeMacFactory.Set("NumHarqProcess", UintegerValue(20));
    ...
    NetDeviceContainer relayUeNetDev = nrHelper->InstallUeDevice(relayUeNodes, allBwps, nrUeMacFactories);  

    //IMP: Add feature to print the NumHarqProcessVariable
    //cout << relayUeNetDev.GetN() << endl;
    //Ptr<NetDevice> nih_netDev = relayUeNetDev.Get(0);
    //Ptr<NrUeNetDevice> nrNetDevice = DynamicCast<NrUeNetDevice> (nih_netDev);
    //Ptr<NrUeMac> mac = nrNetDevice->GetMac(1);
    //uint8_t numHarqProcess = mac->GetNumHarqProcess();
    //cout << endl;
    //cout << "**********************************" << endl;
    //cout << static_cast<unsigned int>(numHarqProcess) << endl;
    //cout << "**********************************" << endl;
    //cout << endl;
    //
  ...
  //NOTE: The issue was that it was saying the enable sensing attribute was an invalid attribute for the NrUeMac
  //NEW CODE START
  ...
  //NOTE: Compared the nr-prose-discovery.cc example file that existed in the old code and the new code
  //NOTE: old one used the Default while the new one used FixedMcs - the issue was that it was not recogizing the Default class 
  //NEW CODE START
	  //Set the SL scheduler attributes
	  nrSlHelper->SetNrSlSchedulerTypeId(NrSlUeMacSchedulerFixedMcs::GetTypeId());
	  nrSlHelper->SetUeSlSchedulerAttribute ("Mcs", UintegerValue (slMcs));
  //NEW CODE END
  ...
  //IMP: check if harqEnalbled = true requires additional param
  //IMP: Again other files seem to set a value for m_pdb
  ...
  // Tasks:
  // 1 - Add names to the nodes
  // 2 - Add logical links - need the logical-links branch of the gui to see these
  // 3 - Graphs, charts and tracings to be added - same as the pngs generated
#ifdef HAS_NETSIMULYZER
  std::string outputFileName = "netsimulyzer-wfpst-example.json";
  ...
  auto relayRemoteColor = netsimulyzer::DARK_ORANGE_VALUE;
  auto offNetColor = netsimulyzer::DARK_PINK_VALUE;

  // Add area surrounding the on-network team
  auto onNetArea = CreateObject<netsimulyzer::RectangularArea> (orchestrator, Rectangle{._xMin=395, ._xMax=402, ._yMin=-25, ._yMax=-15});
  onNetArea->SetAttribute("Border", EnumValue(netsimulyzer::RectangularArea::DrawMode::Solid));
  onNetArea->SetAttribute("BorderColor", onNetColor);

  // Add area surrounding the off-network team
  auto offNetArea = CreateObject<netsimulyzer::RectangularArea> (orchestrator, Rectangle{._xMin=425, ._xMax=432, ._yMin=-58, ._yMax=-48});
  offNetArea->SetAttribute("Border", EnumValue(netsimulyzer::RectangularArea::DrawMode::Solid));
  offNetArea->SetAttribute("BorderColor", offNetColor);

  // Add area surrounding the remote ues
  auto remoteArea = CreateObject<netsimulyzer::RectangularArea> (orchestrator, Rectangle{._xMin=428, ._xMax=434, ._yMin=-96, ._yMax=-88});
  remoteArea->SetAttribute("Border", EnumValue(netsimulyzer::RectangularArea::DrawMode::Solid));
  remoteArea->SetAttribute("BorderColor", relayRemoteColor);

  // Add area surrounding the relay ues
  auto relayArea = CreateObject<netsimulyzer::RectangularArea> (orchestrator, Rectangle{._xMin=395, ._xMax=399, ._yMin=-102, ._yMax=-98});
  relayArea->SetAttribute("Border", EnumValue(netsimulyzer::RectangularArea::DrawMode::Solid));
  relayArea->SetAttribute("BorderColor", relayRemoteColor);

  //IMP: might need () instead of {}
  netsimulyzer::NodeConfigurationHelper nodeConfigHelper{orchestrator};
  nodeConfigHelper.Set("Model", netsimulyzer::models::SMARTPHONE_VALUE);
  nodeConfigHelper.Set("Scale", DoubleValue(2.5));
  ...
  nodeConfigHelper.Set("HighlightColor",
                  netsimulyzer::OptionalValue<netsimulyzer::Color3>{netsimulyzer::DARK_PINK});
  for (uint32_t i = 0; i < offNetUeNodes.GetN(); i++)
  {
    //nodeConfigHelper.Set("Name", StringValue("OffNet - Node " + std::to_string(offNetUeNodes.Get(i)->GetId())));
    auto configContainer = nodeConfigHelper.Install(offNetUeNodes.Get(i));
  }

  // Height of gNb is 30. So the tower in the visualizer is set on the ground with a height of 30
  netsimulyzer::NodeConfigurationHelper nodeConfigHelperTower{orchestrator};
  nodeConfigHelperTower.Set("EnableLabel", BooleanValue(false));
  nodeConfigHelperTower.Set("Model", netsimulyzer::models::CELL_TOWER_VALUE);
  nodeConfigHelperTower.Set("HighlightColor", netsimulyzer::OptionalValue<netsimulyzer::Color3>{netsimulyzer::BLACK});
  nodeConfigHelperTower.Set("Height", netsimulyzer::OptionalValue<double>(30));
  nodeConfigHelperTower.Set("Offset", Vector3DValue(Vector3D(0, 0, -30)));
  //nodeConfigHelperTower.Set("Name", StringValue("gNB"));
  nodeConfigHelperTower.Install(gNbNodes);

  netsimulyzer::BuildingConfigurationHelper buildingConfigHelper(orchestrator);
  buildingConfigHelper.Install(buildings);
  Ptr<Building> door = CreateObject<Building>();
  door->SetBoundaries(Box{._xMIn=409.9, ._xMax=410.1, ._yMin=-102.0, ._yMax=-98.0, ._zMin=0.0, ._zMax=5.0});
  buildingConfigHelper.Install(door);
  
  //NOTE: logical links can only be added to any two devices at a give time
  // Logical links attributes to be set - Id, Color, Orchestrator
  // id - unique id of the logical link 
  ...
  //linkGroup(linkHelper, offNetUeNodes);
  //linkAllToNode(linkHelper, gNbNodes.Get(0), inNetUeNodes);
  //linkAllToNode(linkHelper, relayUeNodes.Get(0), remoteUeNodes);
  //linkHelper.Link(relayUeNodes.Get(0), gNbNodes.Get(0));
  ...
  */
  ...
  //NetSimMcpttPttBasicGroupVisualizer offNetTracer;
  //offNetTracer.SetUp(orchestrator, offNetClientApps);

  //NetSimMcpttPttBasicGroupVisualizer inNetTracer;
  //inNetTracer.SetUp(orchestrator, inNetClientApps);

  //NetSimMcpttPttBasicGroupVisualizer relayTracer;
  //relayTracer.SetUp(orchestrator, relayClientApps);
...
Ptr<McpttTraceHelper> mcpttTraceHelper = CreateObject<McpttTraceHelper> ();
  ...
  //accessTimeTimelineCollection->SetAttribute("Name",
  //                                           StringValue("Access time - Timeline - All calls"));
  ...
  //m2eLatencyTimelineCollection->SetAttribute("Name",
  //                                           StringValue("M2E latency - Timeline - All calls"));
  ...
  // IMP: As of now shall only add the curves for onNet and relay
  //NetSimulyzerMcpttMetricsTracer mcpttMetricsTracer;
  //                                                  
  //// the order is important. teamId-1 corresponds to on-network
  //// the trace sink uses this ordering and thus must be stuck to
  //// alternatively use a map with keys as string to avoid ordering issue - TODO??
  //std::string teams[] = {"on network", "relay + remote", "off network"};
  //netsimulyzer::Color3Value colors[] = {onNetColor, relayRemoteColor, offNetColor};

  //for (uint16_t teamId = 1; teamId <= 3; teamId++)
  //{
  //    // ECDFs for Access time
  //    auto groupAccessTimeEcdf = CreateObject<netsimulyzer::EcdfSink>(
  //        orchestrator,
  //        "Access time - eCDF - " + teams[teamId-1]);
  ...
  //    groupAccessTimeEcdf->GetSeries()->SetAttribute("Color", colors[teamId-1]);
  //    //groupAccessTimeEcdf->SetConnectionType(netsimulyzer::XYSeries::ConnectionType::None);
  //    mcpttMetricsTracer.m_accessTimeEcdfPerCall.insert(
  //        std::pair<uint32_t, Ptr<netsimulyzer::EcdfSink>>(teamId, groupAccessTimeEcdf));
  //    accessTimeEcdfCollection->Add(groupAccessTimeEcdf->GetSeries());

  //    // ECDFs for M2E latency                    
  //    auto groupM2eLatencyEcdfs = CreateObject<netsimulyzer::EcdfSink>(
  //        orchestrator,                           
  //        "M2E latency - eCDF - " + teams[teamId - 1]);
  ...
  //    groupM2eLatencyEcdfs->GetSeries()->SetAttribute("Color", colors[teamId-1]);
  //    //groupM2eLatencyEcdfs->SetConnectionType(netsimulyzer::XYSeries::ConnectionType::None);
  //    mcpttMetricsTracer.m_m2eLatencyEcdfPerCall.insert(
  //        std::pair<uint32_t, Ptr<netsimulyzer::EcdfSink>>(teamId, groupM2eLatencyEcdfs));
  ...
  //        "Name",
  //        StringValue("Access time - Timeline - " + teams[teamId-1]));
  //    groupAccessTimeTimeline->SetAttribute("Color", colors[teamId-1]);
  ...
  //    mcpttMetricsTracer.m_accessTimeTimelineSeriesPerCall.insert(
  //        std::pair<uint32_t, Ptr<netsimulyzer::XYSeries>>(teamId, groupAccessTimeTimeline));
  ...
  //        "Name",
  //        StringValue("M2E latency - Timeline - " + teams[teamId-1]));
  //    groupM2eLatencyTimeline->SetAttribute("Color", colors[teamId-1]);
  ...
  //    mcpttMetricsTracer.m_m2eLatencyTimelineSeriesPerCall.insert(
  //        std::pair<uint32_t, Ptr<netsimulyzer::XYSeries>>(teamId, groupM2eLatencyTimeline));
  //    m2eLatencyTimelineCollection->Add(groupM2eLatencyTimeline);
  //}

  //mcpttTraceHelper->TraceConnectWithoutContext(
  //    "AccessTimeTrace",
  ...
  //NetSimMcpttPttBasicGroupCharter basicGrpTracer;
  //basicGrpTracer.SetUp(orchestrator);
  //basicGrpTracer.AddGroup(offNetClientApps, 3, netsimulyzer::DARK_PINK);
  //basicGrpTracer.AddGroup(inNetClientApps, 1, netsimulyzer::DARK_BLUE);
  //basicGrpTracer.AddGroup(relayClientApps, 2, netsimulyzer::DARK_ORANGE);

//  std::cout << "Offnet:" << std::endl;
//  for (uint32_t i = 0; i < offNetClientApps.GetN(); i++)
//  {
//    auto pttApp = DynamicCast<psc::McpttPttApp>(offNetClientApps.Get(i));
//    ObjectMapValue callMap; 
//    pttApp->GetAttribute("Calls", callMap);
//
//    for (auto it = callMap.Begin(); it != callMap.End(); it++)
//    {
//      auto call = DynamicCast<psc::McpttCall>(it->second);
//      std::cout << "Call id:" << std::to_string(call->GetCallId()) << std::endl;
//
//      auto callMachine = DynamicCast<McpttCallMachineGrp>(call->GetCallMachine());
//      std::cout << "Grp id: " << std::to_string(callMachine->GetGrpId().GetGrpId()) << std::endl;
//    }
//  }
//
//  std::cout << "\nInNet: " << std::endl;
//  for (uint32_t i = 0; i < inNetClientApps.GetN(); i++)
//  {
//    auto pttApp = DynamicCast<psc::McpttPttApp>(inNetClientApps.Get(i));
//    ObjectMapValue callMap; 
//    pttApp->GetAttribute("Calls", callMap);
//
//    for (auto it = callMap.Begin(); it != callMap.End(); it++)
...
//  }
//
//  std::cout << "\nRelayRemotes: " << std::endl;
//  for (uint32_t i = 0; i < relayClientApps.GetN(); i++)
//  {
//    auto pttApp = DynamicCast<psc::McpttPttApp>(relayClientApps.Get(i));
//    ObjectMapValue callMap; 
//    pttApp->GetAttribute("Calls", callMap);
//
//    for (auto it = callMap.Begin(); it != callMap.End(); it++)
...
//return -1;
  ...
  //NOTE: The code update was found by comparing the different versions of nr-prose-unicast-single-link files
  //NOTE: The issue here was that the GetSlUeService method was not found during compiler time
